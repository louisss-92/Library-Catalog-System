  ...
  return (
    <div className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
      <button onClick={toggleSidebar} className="collapse-btn">
        {collapsed ? (
          <IconLayoutSidebarRightCollapse className="toggle-icon" color='#ffc683'/>
        ) : (
          <IconLayoutSidebarLeftCollapse className="toggle-icon" color='#ffc683'/>
        )}
      </button>
      ...
      <div className='buttons'>
        ...
        <button onClick={() => onSelectPage('Registration')} className='functions'>
        ...
        <Popover showArrow backdrop="transparent" placement="right">
          <PopoverTrigger>
            <Button><IconUserCog size={30}/> Admin</Button>
          </PopoverTrigger>
          <PopoverContent>
            <div className="grid grid-cols-1 space-y-1 p-2">
              <h3 className="text-small font-bold mb-1">Log in as a?</h3>
              <Button className="p-1 text-sm" onPress={() => setRegistrationOpen(true)}>Admin Help</Button>
              <Button className="p-1 text-sm" onPress={() => setLibraryOpen(true)}>Library Help</Button>
              <Button className="p-1 text-sm" onPress={() => setAccountOpen(true)}>Registration Help</Button>
              <Button className="p-1 text-sm" onPress={() => setGeneralOpen(true)}>More Help</Button>
            </div>
          </PopoverContent>
        </Popover>
      ...
      {/* Modals */}
      <Modal isOpen={isRegistrationOpen} onOpenChange={setRegistrationOpen} backdrop="opaque" size="1xl">
        <ModalContent>
        {(onClose) => (
            <>
      ...
      <Modal isOpen={isLibraryOpen} onOpenChange={setLibraryOpen} backdrop="opaque" size="1xl">
        <ModalContent>
        {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">Log in</ModalHeader>
      ...
      <Modal isOpen={isAccountOpen} onOpenChange={setAccountOpen} backdrop="opaque" size="1xl">
        <ModalContent>
        {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">Log in</ModalHeader>
      ...
      <Modal isOpen={isGeneralOpen} onOpenChange={setGeneralOpen} backdrop="opaque" size="1xl">
        <ModalContent>
        {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">Log in</ModalHeader>
